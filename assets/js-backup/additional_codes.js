...
jQuery(document).ready(function ($) {
  ...
  // Career Page Interactivity
  $(document).ready(function() {
    if ($('#jobTableBody').length) {
      // 1. Dynamic cloning of badges to the top of detail-content for mobile
      $('.job-row').each(function() {
        var $row = $(this);
        var targetSelector = $row.data('target');
        var $detailContent = $(targetSelector).find('.detail-content');
        if ($detailContent.length && !$detailContent.find('.mobile-only-badges').length) {
          var expVal = $row.find('.job-badge.exp').text();
          var locVal = $row.find('.job-badge.loc').text();
          var $mobileBadges = $(
            '<div class="mobile-only-badges d-sm-none mb-3">' +
              '<span class="job-badge exp me-2">' + expVal + '</span>' +
              '<span class="job-badge loc">' + locVal + '</span>' +
            '</div>'
          );
          $detailContent.prepend($mobileBadges);
        }
      });

      // 2. Expand/Collapse Job Details
      $('#jobTableBody').on('click', '.job-row', function() {
        const $row = $(this);
        const targetSelector = $row.data('target');
        const $detailRow = $(targetSelector);
        const wasExpanded = $row.hasClass('expanded');

        // Collapse all other rows
        $('#jobTableBody').find('.job-row').not($row).removeClass('expanded');
        $('#jobTableBody').find('.detail-row').not($detailRow).each(function() {
          const $otherDetail = $(this);
          if ($otherDetail.hasClass('expanded-transition')) {
            $otherDetail.removeClass('expanded-transition');
            setTimeout(() => {
              const targetRowId = $otherDetail.attr('id');
              const $correspondingJobRow = $('#jobTableBody').find(`.job-row[data-target="#${targetRowId}"]`);
              if (!$correspondingJobRow.hasClass('expanded')) {
                $otherDetail.hide();
              }
            }, 500);
          }
        ...
        if (!wasExpanded) {
          ...
          $detailRow.show();
          $detailRow[0].offsetHeight; // force reflow
          $detailRow.addClass('expanded-transition');
        } else {
          $row.removeClass('expanded');
          $detailRow.removeClass('expanded-transition');
          setTimeout(() => {
      ...
      // 3. Apply Now Button Action inside Detail Row
      $('#jobTableBody').on('click', '.btn-apply-now', function(e) {
        e.stopPropagation(); // Avoid triggering parent row click
        const title = $(this).data('title');
        $('#selectJobTitle').val(title);
        
        if ($.fn.niceSelect) {
          $('#selectJobTitle').niceSelect('update');
        }

        $('html, body').animate({
          scrollTop: $('#applicationFormSection').offset().top - 120
        }, 800);
      });

      // 4. Also hook click directly on the main button inside job-row to expand/collapse details
      $('#jobTableBody').on('click', '.btn-details', function(e) {
        e.stopPropagation();
        $(this).closest('.job-row').trigger('click');
      });

      // 5. Custom File Input naming
      ...
      $('#btnSendOtp').on('click', function() {
        ...
        $('#otpVerifContainer').slideDown(300);
        alert('Mock OTP code "1234" sent to: ' + emailVal);
      });
      ...
      $('#btnVerifyOtp').on('click', function() {
        ...
        if (code === generatedOtp) {
          isOtpVerified = true;
          $('#otpStatus').text('Verified ✓').css('color', '#10b981');
          $('#otpInput').prop('disabled', true);
          $(this).prop('disabled', true);
        } else {
          $('#otpStatus').text('Invalid OTP. Try again.').css('color', '#ef4444');
        }
      });
      ...
      function generateCaptcha() {
        ...
        captchaResult = n1 + n2;
        $('#captchaQuestion').text(`${n1} + ${n2}`);
        $('#captchaAnswer').val('');
      }
      
      generateCaptcha();
      $('#btnRefreshCaptcha').on('click', generateCaptcha);
      ...
      $('#careerForm').on('submit', function(e) {
        ...
        const ans = parseInt($('#captchaAnswer').val().trim(), 10);
        ...
        const $response = $('#formResponseMessage');
        $response.html(`
          <div class="alert alert-success mt-3" role="alert">
            Thank you, ${$('#fullName').val()}! Your application for "${$('#selectJobTitle').val()}" has been successfully submitted. We will get in touch shortly.
          </div>
        `);
