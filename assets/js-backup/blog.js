...
document.addEventListener('DOMContentLoaded', function () {
  const grid = document.querySelector('.blog-section');
  if (!grid) return;
  ...
  async function renderBlogGrid(blogs) {
    const htmlChunks = await Promise.all(blogs.map(async (blog, index) => {
      ...
      return `
                <div class="grc-3 grc-md-12 grc-lg-6 grc-xl-4 ">
                    <div class="features-card blog-section-5__item h-100 fl f-col p-3">
                        <span class="position-relative px-2 py-3 w-auto" style="font-size: 14px; color: var(--primary); font-weight: 500;">
                            ${displayDate} // ${readTime}
                        </span>
                        <h3 class="blog-title original-black mb-4">
